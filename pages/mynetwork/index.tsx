...
const Network = ({ user }) => {
   ...
   const [state, setState] = useState({
      connectionRequesters:
         user_profile.connection_request_connection_request_requested_idTouser_profile.map(
            (item) => {
               let r =
                  item.user_profile_connection_request_initiator_idTouser_profile
               r['invitationMessage'] = item.invitation_message
               return r
            }
         ),
      connections:
         user_profile.connection_connection_user_profile_startTouser_profile.map(
            (item) =>
               item.user_profile_connection_user_profile_endTouser_profile
         ),
   })
   ...
   const onConnectionRequestDecisionHandler = async (
      ...
      if (res.status === 200) {
         ...
         let newState = {
            ...state,
            connectionRequesters: state.connectionRequesters.filter(
               (r) => r.id !== targetUserProfileId
            ),
         }
         if (decision === 'accept') {
            newState = {
