...
const Home = (props) => {
    ...
    const onConferenceFilterClickHandler = ({ name }) => {
        if (filterState.conferences.includes(name))
            setFilterState({
                ...filterState,
                conferences: filterState.conferences.filter(
                    (item) => item !== name
                ),
            })
        else {
            setFilterState({
                ...filterState,
                conferences: [...filterState.conferences, name],
            })
        }
    }
    ...
    if (status === 'authenticated') {
        const userProfiles = props.userProfiles.map((userProfile) => ({
            ...userProfile,
        ...
        const skillLabelSelectOptions = Object.keys(
            columnNameToTagTextMapping
        ).map((dataKey) => ({
            value: dataKey,
            ...
            <Flex direction={'column'}>
                ...
                <Flex direction={'row'} wrap={'wrap'}>
                    <Text
                        ...
                        pr={'2'}
                    >
                        Filter on conferences: (coming soon...)
                    </Text>
                    {props.conferences.map((conf) =>
                        filterTag({
                            name: conf.id,
                            label: conf.conference_name,
                            isChecked: filterState.conferences.includes(
                                conf.id
                            ),
                        })
                    )}
                ...
                <Grid
                    templateColumns={
                        isLargerThan1280 ? 'repeat(3, 1fr)' : 'repeat(2, 1fr)'
                    }
                    gap={3}
                >
                    {userProfiles
                        .filter(
                            (userProfile) =>
                                (filterState.conferences.length === 0 ||
                                    userProfile.conference_ids.some(
                                        (r) =>
                                            filterState.conferences.indexOf(
                                                r
                                            ) >= 0
                                    )) &&
                ...
                >
                    ...
                    >
                        <Stack spacing={4}>
                            ...
                            >
                                Claim your all-in-one place web3 online profile,
                                tell your story, and connect with an amazing
                                network of the web3 buiders.
                            </Text>
                        </Stack>
                        {/* <Link
