  ...
  if (active && payload && payload.length) {
    return (
      <Box bg={"white"}>
        ...
        <Text>
          Investment:{" "}
          {Math.round((payload[0].payload.investment - 10) * 100) / 10} ETH
        </Text>
        <Text>Return: {Math.round(payload[0].payload.roi * 10) / 10} %</Text>
        <Text>
          Gain:{" "}
          {Math.round(
            (payload[0].payload.investment * payload[0].payload.roi) / 10 / 10
          )}{" "}
          ETH
        </Text>
      </Box>
    ...
    <Stack direction={"column"} maxW={"100%"}>
      ...
      <Flex direction={"column"}>
        ...
        <Text fontSize={16} fontWeight={"bold"} pb={4}>
          {trades.filter((t) => t.roi > 0).length} out of {trades.length} trades
          profitable (
          {Math.round(
            (trades.filter((t) => t.roi > 0).length / trades.length) * 1000
          ) / 10}
          %)
        </Text>
        <Box overflow="scroll">
          ...
          >
            ...
            />
            <YAxis dataKey="roi" unit="%" domain={["auto", "auto"]}>
              <Label
                value="Trade Return"
                angle={-90}
                offset={5}
